...
const PersonnelManager = ({ showToast, darkMode }: { showToast: any, darkMode: boolean }) => {
    ...
    const handleExportWord = () => {
        ...
        rollingIncidents.forEach((inc: any, index: number) => {
            activePoints += parseInt(inc.count) || 0; const d = new Date(inc.date);
                ...
                <div className="fixed inset-0 z-[6000] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in zoom-in-95">
                    <div className={`rounded-2xl w-full max-w-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh] ${darkMode ? 'bg-slate-800 text-white border border-slate-700' : 'bg-white text-black'}`}>
                        <div className={`p-6 border-b flex justify-between items-center ${darkMode ? 'bg-slate-900 border-slate-700' : 'bg-slate-50 border-slate-200'}`}>
                            <div><h3 className={`text-2xl font-black uppercase ${darkMode ? 'text-[#ef7c00]' : 'text-[#002d72]'}`}>{selectedEmp.name}</h3><p className={`text-xs font-bold ${darkMode ? 'text-slate-400' : 'text-slate-500'}`}>Total Occurrences: <span className="text-red-500">{selectedEmp.totalOccurrences || 0}</span></p></div>
                            <div className="flex gap-2 items-center"><button onClick={handleExportWord} className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-black text-[10px] uppercase shadow transition-colors">📄 Export Notice</button><button onClick={()=>handleDeleteEmployee(selectedEmp.id, selectedEmp.name)} className="px-4 py-2 bg-red-600 hover:bg-red-700 text-white rounded-lg font-black text-[10px] uppercase shadow transition-colors">Delete Employee</button><button onClick={()=>setSelectedEmp(null)} className="text-2xl text-slate-400 hover:text-red-500 ml-2">✕</button></div>
                        </div>
                        <div className="p-6 overflow-y-auto custom-scrollbar">
                            <div className={`p-4 rounded-xl border mb-6 ${darkMode ? 'bg-slate-900 border-slate-700' : 'bg-blue-50/50 border-blue-100'}`}>
                                ...
                                <div className="grid grid-cols-2 gap-4 mb-3"><select className={`p-2 border rounded font-bold text-xs outline-none focus:border-[#ef7c00] ${inputClass}`} value={incData.type} onChange={e=>setIncData({...incData, type:e.target.value})}><option>Sick</option><option>FMLA</option><option>Failure to Report</option><option>Late Reporting</option><option>NC/NS</option><option>Vacation</option><option>Bereavement</option><option>Other</option></select><input type="number" className={`p-2 border rounded font-bold text-xs outline-none focus:border-[#ef7c00] ${inputClass}`} placeholder="Count" value={incData.count} onChange={e=>setIncData({...incData, count:Number(e.target.value)})} /></div>
                                <div className="flex gap-2 mb-3"><input type="date" onClick={handleDateClick} className={`p-2 border rounded font-bold text-xs flex-grow cursor-pointer outline-none focus:border-[#ef7c00] ${inputClass}`} value={incData.date} onChange={e=>setIncData({...incData, date:e.target.value})} /><div className={`p-2 border rounded cursor-pointer font-bold text-xs flex items-center gap-2 transition-colors ${incData.docReceived?(darkMode?'bg-green-900/50 border-green-700 text-green-400':'bg-green-100 border-green-200 text-green-700'):inputClass}`} onClick={()=>setIncData({...incData, docReceived:!incData.docReceived})}><span>Doc?</span>{incData.docReceived && '✓'}</div></div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        <div className={`rounded-2xl shadow-sm border overflow-hidden ${bgClass}`}>
                            ...
                            <div className="overflow-x-auto"><table className="w-full text-left text-xs"><thead className={`font-black uppercase border-b ${darkMode ? 'bg-slate-900 text-slate-400 border-slate-700' : 'bg-white text-slate-400 border-slate-200'}`}><tr><th className="p-3">Month</th><th className="p-3 text-right">Sick</th><th className="p-3 text-right">FMLA</th><th className="p-3 text-right">No Show</th><th className="p-3 text-right">Total</th></tr></thead><tbody className={`divide-y ${darkMode ? 'divide-slate-700' : 'divide-slate-50'}`}>
                                {stats.monthNames.map(month => { const data = stats.monthlyCounts[month] || {}; if (!data.Total) return null; return (<tr key={month} className={darkMode ? 'text-slate-300' : 'text-slate-700'}><td className="p-3 font-bold">{month}</td><td className="p-3 text-right font-mono text-orange-500">{data['Sick'] || 0}</td><td className="p-3 text-right font-mono text-blue-500">{data['FMLA'] || 0}</td><td className="p-3 text-right font-mono text-red-500">{(data['No Call/No Show'] || 0) + (data['Failure to Report'] || 0)}</td><td className="p-3 text-right font-black">{data.Total || 0}</td></tr>); })}
                            </tbody></table></div>
                        </div>
                        <div className={`rounded-2xl shadow-sm border overflow-hidden flex flex-col h-[400px] ${bgClass}`}>
                            <div className={`p-4 border-b flex justify-between items-center ${darkMode ? 'bg-slate-900 border-slate-700' : 'bg-slate-50 border-slate-200'}`}><h3 className={`text-xs font-black uppercase tracking-widest ${darkMode ? 'text-[#ef7c00]' : 'text-[#002d72]'}`}>Employee Roster</h3><input type="text" placeholder="Search Name..." className={`text-xs p-2 border rounded w-32 font-bold outline-none focus:border-[#ef7c00] transition-colors ${inputClass}`} value={rosterSearch} onChange={e=>setRosterSearch(e.target.value)} /></div>
            ...
            {viewMode === 'log' && (
                <div className={`rounded-2xl shadow-lg border flex-grow overflow-hidden flex flex-col ${darkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'}`}>
                    ...
                    <div className="overflow-x-auto flex-grow custom-scrollbar">
                        <div className={`min-w-[700px] border-b p-3 grid grid-cols-12 gap-2 text-[9px] font-black uppercase tracking-widest ${darkMode ? 'bg-slate-800 border-slate-700 text-slate-400' : 'bg-slate-50 border-slate-200 text-slate-500'}`}><div className="col-span-3 cursor-pointer flex items-center hover:text-[#ef7c00] transition-colors" onClick={() => requestSort('employeeName')}>Employee Name <SortArrow columnKey="employeeName" /></div><div className="col-span-2 cursor-pointer flex items-center hover:text-[#ef7c00] transition-colors" onClick={() => requestSort('type')}>Incident Type <SortArrow columnKey="type" /></div><div className="col-span-2 cursor-pointer flex items-center hover:text-[#ef7c00] transition-colors" onClick={() => requestSort('date')}>Date <SortArrow columnKey="date" /></div><div className="col-span-1 text-center cursor-pointer flex items-center justify-center hover:text-[#ef7c00] transition-colors" onClick={() => requestSort('count')}>Count <SortArrow columnKey="count" /></div><div className="col-span-1 text-center cursor-pointer flex items-center justify-center hover:text-[#ef7c00] transition-colors" onClick={() => requestSort('docReceived')}>Doc? <SortArrow columnKey="docReceived" /></div><div className="col-span-2 flex items-center">Notes</div><div className="col-span-1 text-center flex items-center justify-center">Action</div></div>
                        <div className={`min-w-[700px] divide-y ${darkMode ? 'divide-slate-800' : 'divide-slate-100'}`}>
                            {filteredLog.length === 0 ? <div className={`p-10 text-center italic font-bold ${darkMode ? 'text-slate-600' : 'text-slate-400'}`}>No records found.</div> : filteredLog.map((log, i) => (
                                <div key={i} className={`grid grid-cols-12 gap-2 p-3 items-center transition-colors text-xs ${darkMode ? 'hover:bg-slate-800 text-slate-300' : 'hover:bg-blue-50 text-slate-700'}`}><div className={`col-span-3 font-bold cursor-pointer hover:underline ${darkMode ? 'text-[#ef7c00]' : 'text-[#002d72]'}`} onClick={() => setSelectedEmp(personnel.find(p => p.id === log.employeeId))}>{log.employeeName}</div><div className="col-span-2 font-medium"><span className={`px-2 py-0.5 rounded text-[10px] uppercase font-black ${log.type==='Sick'?'bg-orange-500/20 text-orange-500':log.type==='FMLA'?'bg-blue-500/20 text-blue-500':'bg-red-500/20 text-red-500'}`}>{log.type}</span></div><div className="col-span-2 font-mono">{log.date}</div><div className="col-span-1 text-center font-black">{log.count}</div><div className="col-span-1 text-center">{log.docReceived ? '✅' : '❌'}</div><div className="col-span-2 truncate italic opacity-70">{log.notes || '-'}</div><div className="col-span-1 text-center"><button onClick={() => handleDeleteIncident(log.employeeId, log)} className="text-red-500 hover:text-red-400 font-bold">🗑️</button></div></div>
                            ))}
                        </div>
                    </div>
                </div>
...
export default function FleetManager() {
    ...
    const exportExcel = async () => {
        ...
        const cols = [{header:'Bus #',key:'number',width:10},{header:'Status',key:'status',width:15},{header:'Location',key:'location',width:15},{header:'Fault',key:'notes',width:30},{header:'OOS Start',key:'start',width:15}];
