    ...
    const handleResetRoutes = async () => {
        ...
        try {
            ...
            INITIAL_ROUTES.forEach(defaultRoute => {
                ...
                if (existingDoc) {
                    batch.update(doc(db, "route_disposition", existingDoc.id), {
                        planned: defaultRoute.planned,
                        cancelled: defaultRoute.cancelled,
                        unscheduled: defaultRoute.unscheduled
                    });
                }
            });
            await batch.commit();
            showToast("Routes reset to defaults", "success");
        } catch (e) {
    ...
    return (
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 h-full flex flex-col space-y-6">
            <div className="flex justify-between items-end mb-2">
                ...
                <div className="flex gap-2">
                    {isAdmin && routes.length > 0 && (
                        <button onClick={handleResetRoutes} className="px-6 py-2 border border-red-500/30 bg-red-500/10 text-red-500 hover:bg-red-500 hover:text-white rounded-lg text-[10px] font-black uppercase tracking-widest shadow-md transition-colors">Reset Routes</button>
                    )}
                    {isAdmin && routes.length === 0 && (
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                ...
                <div className={`p-4 rounded-2xl border shadow-sm ${bgClass}`}><p className="text-[9px] font-black uppercase opacity-50 mb-1">Delivery %</p><p className={`text-3xl font-black ${totals.percent >= 100 ? 'text-green-500' : 'text-[#ef7c00]'}`}>{totals.percent}%</p></div>
            </div>
