...
import { collection, onSnapshot, query, orderBy, doc, serverTimestamp, addDoc, deleteDoc, getDoc, updateDoc, arrayUnion, increment } from "firebase/firestore";
...
// --- COMPONENT: BUS DETAIL VIEW ---
const BusDetailViewModal = ({ bus, onClose, showToast, darkMode, isAdmin, statusOptions }: { bus: any; onClose: () => void; showToast: any, darkMode: boolean, isAdmin: boolean, statusOptions: any[] }) => {
    return <BusDetailView bus={bus} onClose={onClose} showToast={showToast} darkMode={darkMode} isAdmin={isAdmin} statusOptions={statusOptions} />;
}
...
export default function FleetManager() {
    ...
    return (
        <div className={`flex flex-col min-h-screen font-sans selection:bg-[#ef7c00] selection:text-white transition-colors duration-300 ${darkMode ? 'bg-slate-900 text-slate-100' : 'bg-slate-100 text-slate-900'}`}>
            {toast && <Toast message={toast.msg} type={toast.type} onClose={()=>setToast(null)} />}
            {selectedBusDetail && (<div className="fixed inset-0 z-[6000] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-200"><BusDetailViewModal bus={selectedBusDetail} onClose={() => setSelectedBusDetail(null)} showToast={showToast} darkMode={darkMode} isAdmin={isAdmin} statusOptions={statusOptions} /></div>)}
