...
import { ADMIN_EMAILS, calculateDaysOOS, logHistory } from './utils';
...
export default function FleetManager() {
    ...
    const [inventoryMode, setInventoryMode] = useState<'list' | 'grid' | 'tv' | 'floor'>('grid');
...
const InventoryView = ({ buses, sortedBuses, searchTerm, setSearchTerm, inventoryMode, setInventoryMode, activeFilter, setActiveFilter, sortConfig, requestSort, getStatusType, setSelectedBusDetail, toggleFullScreen, isFullscreen, tvBoardRef, darkMode, isAdmin, showToast, isLocked }: any) => {
    ...
    // --- DRAG AND DROP LOGIC FOR FLOOR MAP ---
    const SHOP_ZONES = [
        { id: 'Bay 1', type: 'bay' }, { id: 'Bay 2', type: 'bay' }, { id: 'Bay 3', type: 'bay' },
        { id: 'Bay 4', type: 'bay' }, { id: 'Bay 5', type: 'bay' }, { id: 'Bay 6', type: 'bay' },
        { id: 'Pit 1', type: 'pit' }, { id: 'Pit 2', type: 'pit' }, { id: 'Body Shop', type: 'special' }
    ];

    const handleDragStart = (e: React.DragEvent, busNumber: string) => {
        e.dataTransfer.setData("busNumber", busNumber);
    };

    const handleDrop = async (e: React.DragEvent, targetLocation: string) => {
        e.preventDefault();
        const busNumber = e.dataTransfer.getData("busNumber");
        if (busNumber) {
            try {
                await setDoc(doc(db, "buses", busNumber), { location: targetLocation }, { merge: true });
                await logHistory(busNumber, "MOVED", `LOC: Dragged to ${targetLocation}`, auth.currentUser?.email || 'Unknown');
                showToast(`Unit #${busNumber} moved to ${targetLocation}`, 'success');
            } catch (err) {
                console.error(err);
                showToast("Failed to move bus", 'error');
            }
        }
    };
            ...
            <div className={`rounded-3xl border shadow-xl overflow-hidden backdrop-blur-xl flex-grow flex flex-col ${darkMode ? 'bg-slate-900/50 border-slate-800/50' : 'bg-white/80 border-slate-200/50'}`}>
                ...
                {inventoryMode === 'floor' && (
                    <div className="flex h-full w-full p-4 gap-6 overflow-hidden">
                        ...
                        {/* THE YARD (Left Sidebar) */}
                        <div 
                            className={`w-64 flex flex-col rounded-2xl border shadow-inner ${darkMode ? 'bg-slate-900 border-slate-700' : 'bg-slate-100 border-slate-300'}`}
                            onDrop={(e) => handleDrop(e, "The Yard")} 
                            onDragOver={handleDragOver}
                        >
                            <div className={`p-4 border-b ${darkMode ? 'border-slate-700' : 'border-slate-300'}`}>
                                <h3 className={`text-sm font-black uppercase tracking-widest ${darkMode ? 'text-slate-300' : 'text-slate-700'}`}>The Yard</h3>
                                <p className="text-[9px] font-bold opacity-50 uppercase mt-1">Unassigned Down Units</p>
                            </div>
                            <div className="flex-grow overflow-y-auto p-3 space-y-3 custom-scrollbar">
                                {sortedBuses.filter((b:any) => b.status !== 'Active' && !SHOP_ZONES.map(z=>z.id).includes(b.location)).map((b:any) => {
                                    const type = getStatusType(b.status);
                                    return (
                                        <div 
                                            key={b.docId} 
                                            draggable 
                                            onDragStart={(e) => handleDragStart(e, b.number)}
                                            onClick={() => setSelectedBusDetail(b)}
                                            className={`p-3 rounded-xl border cursor-grab active:cursor-grabbing shadow-sm hover:-translate-y-0.5 transition-transform ${darkMode ? 'bg-slate-800 border-slate-600' : 'bg-white border-slate-200'}`}
                                        >
                                            <div className="flex justify-between items-center mb-2">
                                                <span className="font-black text-lg">#{b.number}</span>
                                                <span className={`px-2 py-0.5 rounded text-[8px] font-black uppercase tracking-widest ${getBadgeStyle(type)}`}>{b.status}</span>
                                            </div>
                                            <p className={`text-[10px] font-bold truncate ${darkMode ? 'text-slate-400' : 'text-slate-500'}`}>{b.notes || 'No notes'}</p>
                                        </div>
                                    );
                                })}
                            </div>
                                        ...
                                        >
                                            ...
                                            <div className="flex-grow flex flex-col gap-2">
                                                {parkedBuses.length === 0 ? (
                                                    <div className="flex-grow flex items-center justify-center opacity-20 font-black text-xs uppercase tracking-widest">Drop Unit Here</div>
                                                ) : parkedBuses.map((b:any) => {
                                                    const type = getStatusType(b.status);
                                                    return (
                                                        <div 
                                                            key={b.docId} 
