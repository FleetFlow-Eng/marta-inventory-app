...
const BusTracker = dynamic(() => import('./BusTracker'), { 
  ssr: false,
  loading: () => (
    <div className="flex items-center justify-center h-[85vh] bg-slate-100 rounded-2xl border border-slate-200">
...
const PersonnelManager = ({ showToast }: { showToast: (msg: string, type: 'success'|'error') => void }) => {
    ...
    return (
        <div className="min-h-screen bg-slate-100 text-slate-900 font-sans selection:bg-[#ef7c00] selection:text-white">
            {/* Top Bar Navigation remains inside the main app */}
         ...
         view === 'personnel' ? (isAdmin ? <PersonnelManager showToast={triggerToast} /> : <div className="p-20 text-center text-red-500 font-black">ACCESS DENIED</div>) : (
          <>
            ...
            <div className="mb-6 flex flex-col md:flex-row justify-between items-end gap-4">
                <input type="text" placeholder="Search Unit #..." className="w-full max-w-md pl-4 pr-10 py-3 bg-white border border-slate-200 rounded-lg text-sm font-bold focus:border-[#002d72] text-black outline-none shadow-sm placeholder:text-gray-400" value={searchTerm} onChange={e=>setSearchTerm(e.target.value)} />
                <div className="bg-white border rounded-lg p-1 flex w-full md:w-auto">
                    <button onClick={()=>setInventoryMode('list')} className={`flex-1 md:flex-none px-4 py-1.5 text-[10px] font-black uppercase rounded transition-all ${inventoryMode==='list'?'bg-[#002d72] text-white shadow-md':'text-slate-400 hover:text-[#002d72]'}`}>List</button>
                    <button onClick={()=>setInventoryMode('grid')} className={`flex-1 md:flex-none px-4 py-1.5 text-[10px] font-black uppercase rounded transition-all ${inventoryMode==='grid'?'bg-[#002d72] text-white shadow-md':'text-slate-400 hover:text-[#002d72]'}`}>Grid</button>
                    <button onClick={()=>setInventoryMode('tv')} className={`flex-1 md:flex-none px-4 py-1.5 text-[10px] font-black uppercase rounded transition-all ${inventoryMode==='tv'?'bg-[#ef7c00] text-white shadow-md':'text-slate-400 hover:text-[#ef7c00]'}`}>TV Board</button>
                </div>
            </div>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden min-h-[500px]">
                ...
                {inventoryMode === 'list' && (
                    <div className="overflow-x-auto">
                        <div className="grid grid-cols-10 gap-4 p-5 border-b bg-slate-50/50 text-[9px] font-black uppercase text-slate-400 tracking-widest min-w-[800px]">
                            <div onClick={()=>requestSort('number')} className="cursor-pointer hover:text-[#002d72]">Unit #</div><div>Series</div><div>Status</div><div>Location</div><div className="col-span-2">Fault Preview</div><div>Exp Return</div><div>Act Return</div><div>Days OOS</div>
                        </div>
                ...
                {/* 2. GRID VIEW (DETAILED CARDS) */}
                {inventoryMode === 'grid' && (
                    <div className="p-6 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4 bg-slate-50/50">
                        {sortedBuses.map(b => (
                            <div key={b.docId} onClick={()=>setSelectedBusDetail(b)} className={`p-4 rounded-2xl border-2 flex flex-col cursor-pointer hover:-translate-y-1 transition-all shadow-sm bg-white ${b.status==='Active'?'border-green-200 hover:border-green-400':'border-red-200 hover:border-red-400'}`}>
                                <div className="flex justify-between items-start mb-3">
                                    <span className={`text-2xl font-black ${b.status==='Active'?'text-green-800':'text-red-800'}`}>#{b.number}</span>
                                    <span className={`px-2 py-1 rounded border text-[8px] font-black uppercase ${b.status==='Active'?'bg-green-50 border-green-200 text-green-700':'bg-red-50 border-red-200 text-red-700'}`}>{b.status}</span>
                                </div>
                                <div className="flex items-center gap-1 text-xs font-bold text-slate-600 mb-2 truncate">📍 {b.location || 'Unknown loc'}</div>
                                <div className="text-[10px] text-slate-500 italic line-clamp-2 mb-3 h-8">🔧 {b.notes || 'No active faults recorded.'}</div>
                                {b.status !== 'Active' && <div className="mt-auto text-[10px] font-black text-red-600 bg-red-50 p-1.5 rounded-lg text-center border border-red-100">⏱️ DOWN {calculateDaysOOS(b.oosStartDate)} DAYS</div>}
                            </div>
                        ))}
                    </div>
                )}

                {/* 3. TV BOARD VIEW (DENSE, ONE PAGE) */}
                {inventoryMode === 'tv' && (
                    <div className="p-2 grid grid-cols-4 sm:grid-cols-6 md:grid-cols-8 lg:grid-cols-10 xl:grid-cols-12 2xl:grid-cols-14 gap-1.5 bg-black min-h-[75vh] content-start overflow-hidden">
                        {sortedBuses.map(b => (
                            <div key={b.docId} onClick={()=>setSelectedBusDetail(b)} className={`p-1.5 rounded flex flex-col justify-between cursor-pointer border ${b.status==='Active'?'bg-[#0a2010] border-[#1a4020] text-green-400':'bg-[#2a0a0a] border-[#4a1a1a] text-red-400'}`}>
                                <div className="flex justify-between items-start mb-1">
                                    <span className="text-sm font-black leading-none text-white">#{b.number}</span>
                                    {b.status !== 'Active' && <span className="text-[7px] font-black uppercase leading-none opacity-80">{b.status.substring(0,4)}</span>}
                                </div>
                                <div className="text-[9px] font-bold truncate leading-tight opacity-70 mb-0.5">{b.location || '-'}</div>
                                {b.status !== 'Active' && <div className="text-[9px] font-black leading-none tracking-widest">{calculateDaysOOS(b.oosStartDate)}d OOS</div>}
                            </div>
                        ))}
                    </div>
                )}
