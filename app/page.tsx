...
const logActivity = async (userEmail: string, category: string, target: string, action: string, details: string) => {
    if (!userEmail) return;
    try { 
        await addDoc(collection(db, "activity_logs"), { user: userEmail, category, target, action, details, timestamp: serverTimestamp() }); 
...
const StatusCharts = ({ buses, statusOptions }: { buses: any[], statusOptions: any[] }) => {
    ...
    // Dynamically calculate statuses based on current data
...
const AnalyticsDashboard = ({ buses, showToast }: { buses: any[], showToast: any }) => {
    ...
    useEffect(() => { 
        ...
        const fetchRankings = async () => { 
            try {
                ...
                // Only scan buses currently down or heavily documented to save reads
    ...
    const handleResetMetrics = async () => { 
        ...
        try { 
            ...
            // 1. Wipe Activity Logs in chunks
            const aSnap = await getDocs(collection(db, "activity_logs"));
            const aDocs = aSnap.docs;
            for (let i = 0; i < aDocs.length; i += 250) {
                await Promise.all(aDocs.slice(i, i + 250).map(d => deleteDoc(d.ref).catch(e => { console.error(e); errorCount++; })));
            }
            
            // 2. Wipe Bus History in chunks
            for (const bus of buses) { 
                ...
                for (let i = 0; i < hDocs.length; i += 250) {
                    await Promise.all(hDocs.slice(i, i + 250).map(d => deleteDoc(d.ref).catch(e => { console.error(e); errorCount++; })));
                }
            } 

            // 3. Reset Personnel Incidents in chunks
            const pSnap = await getDocs(collection(db, "personnel"));
            const pDocs = pSnap.docs;
            for (let i = 0; i < pDocs.length; i += 250) {
                await Promise.all(pDocs.slice(i, i + 250).map(d => updateDoc(d.ref, { incidents: [], totalOccurrences: 0 }).catch(e => { console.error(e); errorCount++; })));
            }
